...
{
	// Chucking a new one
	AngleVectors( self->client->ps.viewangles, vforward, vright, vup );
	CalcMuzzlePoint( self, vforward, vright, vup, muzzle, 0 );
...
//---------------------------------------------------------
{
	...
	if ( ent->s.weapon == WP_DISRUPTOR && alt_fire )
	{
		if ( ent->NPC )
		{
			...
			AngleVectors( ent->lastAngles, vforward, vright, vup );
		}
	}
	else if ( ent->s.weapon == WP_ATST_SIDE || ent->s.weapon == WP_ATST_MAIN ) 
	{
		...
		{//NPC, auto-aim at enemy
			...
			AngleVectors (angleToEnemy1, vforward, vright, vup);
		}
	} 
	...
	CalcMuzzlePoint ( ent, vforward, vright, vup, muzzle , 0);
